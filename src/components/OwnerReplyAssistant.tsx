...
export function generateOwnerReplyDraft(
  reviewerName: string,
  reviewText: string,
  rating: number,
  tone: ReplyTone,
...
interface OwnerReplyAssistantProps {
  onBack?: () => void;
}

export default function OwnerReplyAssistant({ onBack }: OwnerReplyAssistantProps) {
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
          <div className="space-y-1">
            ...
            <p className="text-slate-400 text-xs sm:text-sm max-w-xl">
              Draft professional, courteous owner replies to genuine customer reviews. Customer reviews remain 100% untouched.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            {onBack ? (
              <button
                onClick={onBack}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-700 text-xs font-medium cursor-pointer transition-colors"
              >
            ...
            <a
              href="/review"
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 border border-blue-500/30 text-xs font-semibold cursor-pointer transition-colors"
            >
              <span>Customer Review Flow</span>
              <ExternalLink className="w-3 h-3" />
            </a>
          </div>
        </div>

        {/* OWNER REVIEW REPLY ASSISTANT */}
